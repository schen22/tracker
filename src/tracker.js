...
const PuppyTracker = () => {
  ...
  const getHourlyTrends = () => {
    ...
    const hourlyData = Array.from({ length: 24 }, (_, hour) => {
      ...
      const pottyEvents = pottyLogs.filter(
        log =>
  ...
  const getSuccessRateTrend = () => {
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      const successRate =
        dayLogs.length > 0
          ? Math.round((successfulLogs.length / dayLogs.length) * 100)
          : 0;
    ...
    const distribution = [
      {
        ...
        value: recentLogs.filter(
          l => l.type === "pee" && l.location === "outside"
        ).length,
        color: "#3B82F6"
      },
      {
        ...
        value: recentLogs.filter(
          l => l.type === "poop" && l.location === "outside"
        ).length,
        color: "#10B981"
      },
      {
        name: "Pee Accidents",
        value: recentLogs.filter(
          l => l.type === "pee" && l.location === "inside"
        ).length,
        color: "#F87171"
      },
      {
        name: "Poop Accidents",
        value: recentLogs.filter(
          l => l.type === "poop" && l.location === "inside"
        ).length,
        color: "#EF4444"
      }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            ...
            <div className="bg-red-50 p-3 rounded-lg text-center">
              <div className="text-2xl font-bold text-red-600">
                {
                  getTodaysPottyLogs().filter(log => log.location === "inside")
                    .length
                }
              </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          {getHourlyTrends().length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-gray-700 mb-2">
                Hourly Potty Patterns (Last 7 Days)
              </h3>
              <ResponsiveContainer width="100%" height={120}>
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {[...getTodaysPottyLogs(), ...getTodaysActivities()]
              ...
              .map(item => (
                <div
                  key={item.id}
                  className="flex items-center justify-between p-2 rounded border-l-4 border-l-blue-200 bg-gray-50 hover:bg-gray-100 group"
                >
