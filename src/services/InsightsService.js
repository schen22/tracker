    ...
    return Array.from({ length: 24 }, (_, hour) => {
      ...
      return {
        ...
        accidents: hourLogs.filter(log => {
          ...
          return log.type === 'accident' || log.location === 'inside' || log.location === 'crate';
        }).length,
  ...
  getDailyHourlyActivity(date) {
    ...
    return Array.from({ length: 24 }, (_, hour) => {
      ...
      return {
        ...
        feeding: otherActivities.filter(a => a.activity?.toLowerCase().includes('fed') || a.activity?.toLowerCase().includes('food') || a.activity?.toLowerCase().includes('meal')).length,
        play: otherActivities.filter(a => a.activity?.toLowerCase().includes('play')).length,
        training: otherActivities.filter(a => a.activity?.toLowerCase().includes('training') || a.activity?.toLowerCase().includes('train')).length,
        crate: otherActivities.filter(a => a.activity?.toLowerCase().includes('crate')).length,
    ...
    const distribution = [
      { 
        ...
        value: recentLogs.filter(l => {
          ...
          const isSuccessful = l.hasOwnProperty('isSuccessful') ? l.isSuccessful : (l.type !== 'accident' && l.location === 'outside');
      ...
      { 
        ...
        value: recentLogs.filter(l => {
          const isPee = l.type === 'pee';
          const isAccident = l.hasOwnProperty('isAccident') ? l.isAccident : (l.type === 'accident' || l.location === 'inside' || l.location === 'crate');
